...
export function LocationsPage() {
  ...
  const handleLocationSaved = useCallback(
    (location: Location | null, nextIsActive: boolean) => {
      if (!location || locations.length !== 1 || page <= 1) return;

      const leavesCurrentFilter =
        (activityFilter === "active" && !nextIsActive) ||
        (activityFilter === "inactive" && nextIsActive);

      if (leavesCurrentFilter) {
        setPage((currentPage) => Math.max(1, currentPage - 1));
      }
    },
    [activityFilter, locations.length, page],
  );
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      <section className="rounded-3xl border border-[#2f281f] bg-[#111816] p-6 sm:p-8 lg:p-10">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[1fr_auto]">
          ...
          <div className="flex gap-2 sm:gap-3">
            {(["all", "active", "inactive"] as ActivityFilter[]).map((filter) => {
      ...
      <LocationsList
        locations={locations}
        isLoading={isLoading || (isFetching && locations.length === 0)}
        isError={isError}
        error={error || null}
        onRetry={refetch}
